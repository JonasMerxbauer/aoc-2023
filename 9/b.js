...
const returnNextValueAtStartOfLine = (lines) => {
  const newLines = JSON.parse(JSON.stringify(lines));
  let newNumber = 0;
  for (let i = 0; i < lines.length; i++) {
    const currentLine = newLines.pop();

    newNumber = currentLine[0];

    if (i !== lines.length - 1) {
      newLines[newLines.length - 1].unshift(
        newLines[newLines.length - 1][0] - newNumber
      );
    }
  }
...
for (line of data) {
  ...
  const nextValueAtStart = returnNextValueAtStartOfLine(lines);

  nextValues.push(nextValueAtStart);
}
